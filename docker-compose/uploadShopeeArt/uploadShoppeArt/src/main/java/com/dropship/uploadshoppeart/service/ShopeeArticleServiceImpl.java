...
public class ShopeeArticleServiceImpl extends CommonClient implements ShopeeArticleService {
  ...
  public String createShoppeArt(ShoppeArticle shopeeArticle) {
    ResponseEntity<CreateShopeeArtResponse> createShopeeArtRp = restTemplate.exchange(CREATE_SHOPPE_ART_URL, HttpMethod.POST,
        getCreateArtEntity(shopeeArticle), CreateShopeeArtResponse.class);
    return null;
  }
  ...
  private HttpEntity getCreateArtEntity(ShoppeArticle shopeeArticle) {
    ...
    String formatedRequest = request.replace("$ARTICLE_NAME", shopeeArticle.getArtName()).replace("$ARTICLE_DESCRIPTION",
          shopeeArticle.getDescription());
    ;
